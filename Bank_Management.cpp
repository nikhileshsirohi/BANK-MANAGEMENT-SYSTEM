...
#include <unistd.h>
using namespace std;
const string admin_pass = "12345678";
class account
{
	int acc_no;
	char name[50];
...
// Function to set terminal input echoing off
void disableEcho()
{
	struct termios term;
...
// Function to set terminal input echoing on
void enableEcho()
{
	struct termios term;
	tcgetattr(STDIN_FILENO, &term);
...
{
	cout << acc_no << setw(10) << name << setw(7) << acc_type << setw(7) << deposit << setw(10) << ph_no << endl;
}
